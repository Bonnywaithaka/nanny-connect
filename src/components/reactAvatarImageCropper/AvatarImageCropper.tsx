  ...
  onDrop = (evt) => {
    ...
    if (ifImage && file.size <= maxsize) {
      ...
      if (this.props.errorHandler) {
        ...
        this.setState({
          errorMsg: "The size of image is too large than 2mbs",
        });
      }
    }
  };
  ...
  _onMouseDown = (e) => {
    if (this.ifMobile) {
      this.setState((previousState) => ({
        x: e.touches[0].clientX - previousState.relX,
        y: e.touches[0].clientY - previousState.relY,
      }));
      document.addEventListener("touchmove", this._onMove);
      document.addEventListener("touchend", this._onMouseUp);
    } else {
      this.setState((previousState) => ({
        x: e.clientX - previousState.relX,
        y: e.clientY - previousState.relY,
      }));
    ...
    if (HTMLCanvasElement && (!canvas_proto.toBlob || !canvas_proto.toBlobHD)) {
      if (!canvas_proto.toBlob)
        canvas_proto.toBlob = function (callback, type /* , ...args */) {
          ...
          const is_base64 = is_base64_regex.test(
          ...
          if (window.Blob.fake) {
            ...
            if (is_base64) {
              blob = new window.Blob([decode_base64(data)], { type });
            } else {
              blob = new window.Blob([decodeURIComponent(data)], { type });
            }
          }
          callback(blob);
        };
